...
<p>Desde el apartado 3.2 de esta Secci?n 1, tomamos la pregunta objetivo realizada y, adem?s del pertinente test de correlaci?n entre dos variables para contestar la pregunta objetivo, vamos a hacer un an?lisis de regresi?n lineal para ver la posible relaci?n existente entre estas dos variables.</p>

<p> _Para el f?rmaco P y el metabolito M (por separado), para una dosis cualquiera del Panel 1 (10, 20 o 40 mg) ?se puede decir que existe correlaci?n entre la AUC y la Cmax correspondientes? Tomar cada columna como una variable_ </p>

<p>Recordamos los valores que daban los coeficientes no param?tricos de correlaci?n de Spearman y Kendall. Cualquiera de los dos coeficientes se acepta, siempre que tenga un valor lo m?s cercano a 1. </p>

```{r}
cor.test(panel1m$`AUC 20 mg`,panel1m$`Cmax 20 mg`, method = "spearman") #Tomamos el coeficiente rho de Spearman por ser el m?s cercano a 1. Rho  = 0.8857
```

<p> No obstante, probamos a realizar un test no param?trico de correlaci?n entre dos variables (AUC y Cmax de diversas dosis) del f?rmaco P y el metabolito M para detectar alg?n otro valor de rho cercano a 1</p>
...
<p> _Los siguientes fundamentos te?ricos se han obtenido de la siguiente web especializada en estad?stica computacional:_ http://www.sthda.com/english/articles/39-regression-model-diagnostics/ </p>
...
library(broom)
model_panel2m_aug<-augment(model_panel2m) #Valores tabulados de los 4 gr?ficos.

plot(model_panel2m, 1) #Residuals vs fitted. Asunci?n de modelo lineal o no. Aceptable.
...
_El impacto en la lectura de los resultados figura en el apartado 7 de esta secci?n_

<p>**7.(0,5 puntos) A partir de los datos de origen y el estudio realizado, haced una valoraci?n final.** </p>

De los an?lisis realizados en los apartados anteriores se extraer unas conclusiones que, si bien son bastante robustas, dejan entrever la necesidad de hacer an?lisis estad?sticos en mayor n?mero y dimensi?n (de variables)

_Respecto a los apartados 3.1 a 3.4:_ 
Los asunciones previas de normalidad y homocedasticidad (igualdad de varianzas) no se cumplen, y por ello se deben de realizar tests no param?trico para estudiar la correlaci?n entre las variables que nos interese estudiar. 

Solo se realiza el test de Shapiro, sin el test F de la varianza, porque es suficiente con no cumplir uno de los dos criterios para que no se deba hacer ning?n test param?trico (aquellos que se corresponden a una determinada distribuci?n estad?stica) Se propone utilizar el test de Spearman, pero tambi?n es correcto realizar el test de Kendall, ambos indicados para estos contextos.

Podemos ver que la relaci?n entre los par?metros farmacocin?ticos AUC y Cmax, para la dosis y factor estudiado, es casi nula, pero es recomendable ver qu? ocurre con el resto de las dosis y/o factores, pues para la dosis de 20/80/300 mg, con el metabolito, la correlaci?n es estad?sticamente significativa, as? como para otras con el f?rmaco P.

No parece haber correlaci?n entre dosis bajas para el par?metro pk AUC (ni factor P ni M), as? como tampoco la hay entre dosis baja y dosis alta (ni factor P ni M) Para todos ellos, el valor de **p-value > 0.05**, superior al nivel de significancia fijado para el test, lo que no nos permite rechazar la hip?tesis nula fijada para cada test.
...
El an?lisis de las variables de forma aislada o de dos en dos ("pairwise") es un recurso pr?ctico y leve de teor?a, pero **no permite establecer una imagen completa con todas las variables expuestas al mismo tiempo**, que es lo que realmente est? ocurriendo en cualquier proceso biol?gico/qu?mico/f?sico, por lo que las lecturas que pudieran hacerse de estos an?lisis deber?an de ser consideradas como bastante limitadas (por matem?ticamente correctas que sean)

Lo ideal ser?a proseguir, adem?s de con las pruebas de tipo univariante o pairwise vistas durante esta asignatura, con an?lisis multivariante, pero el requisito b?sico para realizar cualquier prueba de este estilo (especialmente los de an?lisis de la varianza multivariable, MANOVA) es que todas las variables estudiadas simult?neamente han de cumplir con la normalidad y ser independientes entre ellas mismas (no correlacionadas) El test multivariable de normalidad no se puede ejecutar por ser la matriz introducida de tipo singular (sin soluci?n), muy probablemente por la correlaci?n probada entre las variables estudiadas. 

_Respecto al an?lisis de regresi?n lineal de la pregunta 6:_
La lectura se puede hacer de los gr?ficos, que se generan del modelo lineal lm presupuesto. Lo que quer?amos saber es si este modelo es el que mejor se adapta al comportamiento que siguen experimentalmente las dos variables escogidas (AUC 80 mg y Cmax 80 mg para el factor M), un hecho que podemos hallar del gr?fico de puntos residuales-puntos ajustados ("residuals vs fitted"), cuya representaci?n refleja que el ajuste lineal no es bueno, y no es el modelo que mejor se adapta; **su gr?fico se desv?a bastante de una l?nea recta** , que ser?a lo que se obtendr?a en caso de relaci?n lineal entre variables. Ser?a conveniente generar otro modelo lineal con el factor P, para contrastar resultados y poder hacer una lectura m?s robusta.

**En general, y como conclusi?n, buena parte de este estudio es recomendable hacerlo con un tama?o muestral mayor, para poder ver si d?ndose la normalidad, al estudiar la varianza, existe homocedasticidad y as? realizar test param?tricos. Aportar?a algo de informaci?n adicional al estudio de forma global, si bien dichos tests no deber?an de reflejar resultados estad?sticamente diferentes a los obtenidos con los tests no param?tricos.**

**Parece tratarse de un f?rmaco con un comportamiento farmacocin?tico peculiar, muy irregular y heterog?neo para el f?rmaco P y algo menos para su metabolito M, aunque con excepciones remarcables ("influential cases") en ambos factores (AUC 20 mg para M, AUC dosis altas para P.)**
